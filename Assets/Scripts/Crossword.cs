    ...
    void Gen(int pos)
    {
        ...
        for (int i = pos; i < _wordsToInsert.Count; i++)
        {
            ...
            if (posi != null)
            {
                ...
                if (posi.third == 0 && inRTL)
                    word = word.Aggregate("", (x, y) => y + x);
                var value = posi.third == 0 ? _hCount : _vCount;
                PutWord(word, posi.first, posi.second, posi.third, value);
                Gen(pos + 1);
                RemoveWord(word, posi.first, posi.second, posi.third);
            }
            else
            {
                Gen(pos + 1);
